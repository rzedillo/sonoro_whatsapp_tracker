...
async def shutdown_orchestrator():
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            orchestrator_health = {
                "orchestrator": {
                    "healthy": self.is_running and self.status == OrchestratorStatus.RUNNING,
                    "status": self.status.value,
                    "uptime": (datetime.utcnow() - self.start_time).total_seconds(),
                }
            }
            ...
            if agent_health_tasks:
                agent_results = await asyncio.gather(*agent_health_tasks, return_exceptions=True)
                
                for i, result in enumerate(agent_results):
                    agent_name = list(self.agents.keys())[i]
                    if isinstance(result, Exception):
                        orchestrator_health[agent_name] = {
                            "healthy": False,
                            "error": str(result)
                        }
                    else:
                        orchestrator_health[agent_name] = result
